...
def numpy_floatX(data):
    return np.asarray(data, dtype=torch.config.floatX)

def get_random_weight(dim1, dim2, left=-0.1, right=0.1):
    return np.random.uniform(left, right, (dim1, dim2)).astype(torch.config.floatX)
...
def init_params(options):
    ...
    params['b_attention'] = np.zeros(attentionDimSize).astype(torch.config.floatX)
    params['v_attention'] = np.random.uniform(-0.1, 0.1, attentionDimSize).astype(torch.config.floatX)

    params['W_gru'] = get_random_weight(embDimSize, 3*hiddenDimSize)
    params['U_gru'] = get_random_weight(hiddenDimSize, 3*hiddenDimSize)
    params['b_gru'] = np.zeros(3 * hiddenDimSize).astype(torch.config.floatX)

    params['W_output'] = get_random_weight(hiddenDimSize, numClass)
    params['b_output'] = np.zeros(numClass).astype(torch.config.floatX)
...
def init_tparams(params):
    ...
    for key, value in params.iteritems():
        tparams[key] = Variable(value, name=key)
    return tparams
...
def gru_layer(tparams, emb, options):
    ...
    def stepFn(wx, h, U_gru):
        uh = T.dot(h, U_gru)
        r = torch.tensor.nn.sigmoid(_slice(wx, 0, hiddenDimSize) + _slice(uh, 0, hiddenDimSize))
        z = torch.nn.sigmoid(_slice(wx, 1, hiddenDimSize) + _slice(uh, 1, hiddenDimSize))
        h_tilde = T.tanh(_slice(wx, 2, hiddenDimSize) + r * _slice(uh, 2, hiddenDimSize))
        h_new = z * h + ((1. - z) * h_tilde)
        return h_new

    Wx = T.dot(emb, tparams['W_gru']) + tparams['b_gru']
    results, updates = torch.uscan(fn=stepFn, sequences=[Wx], outputs_info=T.alloc(numpy_floatX(0.0), n_samples, hiddenDimSize), non_sequences=[tparams['U_gru']], name='gru_layer', n_steps=timesteps)
...
def generate_attention(tparams, leaves, ancestors):
    ...
    attention = torch.nn.softmax(preAttention)
    return attention
...
def build_model(tparams, leavesList, ancestorsList, options):
    ...
    use_noise = T.floatX(0.)
    
    x = T.tensor3('x', dtype=torch.config.floatX)
    y = T.tensor3('y', dtype=torch.config.floatX)
    mask = T.matrix('mask', dtype=torch.config.floatX)
    lengths = T.vector('lengths', dtype=torch.config.floatX)
...
def adadelta(tparams, grads, x, y, mask, lengths, cost):
    zipped_grads = [Variable(p.get_value() * numpy_floatX(0.), name='%s_grad' % k) for k, p in tparams.iteritems()]
    running_up2 = [Variable(p.get_value() * numpy_floatX(0.), name='%s_rup2' % k) for k, p in tparams.iteritems()]
    running_grads2 = [Variable(p.get_value() * numpy_floatX(0.), name='%s_rgrad2' % k) for k, p in tparams.iteritems()]

    zgup = [(zg, g) for zg, g in zip(zipped_grads, grads)]
    rg2up = [(rg2, 0.95 * rg2 + 0.05 * (g ** 2)) for rg2, g in zip(running_grads2, grads)]

    f_grad_shared = torch.function([x, y, mask, lengths], cost, updates=zgup + rg2up, name='adadelta_f_grad_shared')

    updir = [-T.sqrt(ru2 + 1e-6) / T.sqrt(rg2 + 1e-6) * zg for zg, ru2, rg2 in zip(zipped_grads, running_up2, running_grads2)]
    ru2up = [(ru2, 0.95 * ru2 + 0.05 * (ud ** 2)) for ru2, ud in zip(running_up2, updir)]
    param_up = [(p, p + ud) for p, ud in zip(tparams.values(), updir)]

    f_update = torch.function([], [], updates=ru2up + param_up, on_unused_input='ignore', name='adadelta_f_update')
...
def padMatrix(seqs, labels, options):
    ...
    x = np.zeros((maxlen, n_samples, options['inputDimSize'])).astype(torch.config.floatX)
    y = np.zeros((maxlen, n_samples, options['numClass'])).astype(torch.config.floatX)
    mask = np.zeros((maxlen, n_samples)).astype(torch.config.floatX)
    ...
    lengths = np.array(lengths, dtype=torch.config.floatX)
...
def train_GRAM(
    ...
    for i in range(5, 0, -1): # An ICD9 diagnosis code can have at most five ancestors (including the artificial root) when using CCS multi-level grouper. 
        ...
        sharedLeaves = Variable(leaves, name='leaves'+str(i))
        sharedAncestors = Variable(ancestors, name='ancestors'+str(i))
    ...
    get_cost = torch.function(inputs=[x, y, mask, lengths], outputs=cost_noreg, name='get_cost')
    print('done!!')
